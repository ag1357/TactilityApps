...
int main(int argc, char** argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: world_viewer product.cws [--headless]\nWASD move, arrows turn, E lift/portal, Escape exit\n");
        return 2;
    }
    FILE* f = fopen(argv[1], "rb");
    if (!f) return 2;
    uint8_t data[48 + 64 * WS_CAP + 8 * WS_LINK_CAP];
    size_t n = fread(data, 1, sizeof(data), f);
    int extra = fgetc(f);
    fclose(f);
    WsError err = ws_load(&recipe, data, n);
    if (extra != EOF || err != WS_OK) {
    ...
    WsTraveler t = {0};
    t.at.scope = UINT16_MAX;
    for (uint16_t i = 0; i < recipe.count; i++)
        if ((recipe.modules[i].flags & WS_WALK) && !(recipe.modules[i].flags & WS_INTERIOR)) {
            WsModule m;
            ws_materialize(&recipe, i, &m);
            t.at.pos = (WsPos) {m.pos.x, m.pos.y + 300, m.pos.z};
            break;
        }
    int running = 1, yaw = 0, frames = 0;
    double sum = 0;
    WsMetrics metrics = {0};
    while (running) {
        SDL_Event e;
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                if (e.key.keysym.sym == SDLK_e) ws_use_link(&recipe, &t);
            }
        }
        const Uint8* k = SDL_GetKeyboardState(0);
        yaw = (yaw + (k[SDL_SCANCODE_RIGHT] - k[SDL_SCANCODE_LEFT]) * 3 + 360) % 360;
        float a = yaw * .01745329252f;
        int forward = k[SDL_SCANCODE_W] - k[SDL_SCANCODE_S], side = k[SDL_SCANCODE_D] - k[SDL_SCANCODE_A];
        if (!t.remaining) ws_move(&recipe, &t.at, (int32_t)(80 * (forward * sinf(a) + side * cosf(a))), (int32_t)(80 * (forward * cosf(a) - side * sinf(a))));
        ws_travel_tick(&t, 16);
        uint64_t start = SDL_GetPerformanceCounter();
        render_world(&frame, &recipe, t.at, yaw, &metrics);
        sum += (SDL_GetPerformanceCounter() - start) * 1000. / SDL_GetPerformanceFrequency();
        draw_panel(&frame, 0, 0, W, 11, 0);
        draw_text(&frame, 2, 1, "WORLD SDK | WASD E LIFT/PORTAL", 0xffff, 0);
        SDL_UpdateTexture(texture, 0, frame.pixels, W * 2);
        SDL_RenderCopy(display, texture, 0, 0);
        SDL_RenderPresent(display);
        frames++;
        if (headless && frames == 120) running = 0;
