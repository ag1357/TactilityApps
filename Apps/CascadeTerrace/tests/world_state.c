...
static WsContext ctx(uint32_t player,uint16_t target,int server) {
    WsModule m;ws_materialize(&r,target,&m);return (WsContext){player,{{m.pos.x,m.pos.y+300,m.pos.z},(m.flags&WS_INTERIOR)?target:UINT16_MAX},(uint8_t)server};
}
...
static void setup(void) {
    ws_state_init(&s,&r);assert(ws_join(&s,101)==WS_OK);assert(ws_join(&s,102)==WS_OK);
    /* Explicit qualification provisioning, not a client authority bypass. */
    s.players[0].inventory[1]=60000;s.players[1].inventory[1]=100;
...
int main(void) {
    ...
    d=ws_merge(&s,&base,&r,ctx(102,4,0),repair);CHECK(d.status==WS_OK&&d.rewarded&&!d.world_changed&&s.players[1].credits==10); /* B */
    WsOperation damage=op(WS_DAMAGE,4,2);CHECK(ws_apply(&s,&r,ctx(101,4,1),damage).status==WS_OK);
    d=ws_merge(&s,&base,&r,ctx(102,4,0),repair);CHECK(d.status!=WS_OK&&s.entities[4].health==0); /* C */
    repair=op(WS_REPAIR,4,3);CHECK(ws_apply(&s,&r,ctx(101,4,0),repair).status==WS_OK&&s.players[0].credits==20);
    setup();base=s;WsOperation aid=op(WS_AID,15,1);WsOperation kill=op(WS_KILL,15,1);
    CHECK(ws_apply(&s,&r,ctx(102,15,1),kill).status==WS_OK);
    d=ws_merge(&s,&base,&r,ctx(101,15,0),aid);CHECK(d.status==WS_OK&&d.historical&&d.rewarded&&!s.entities[15].alive); /* D */
    CHECK(ws_merge(&s,&base,&r,ctx(101,15,0),aid).status!=WS_OK);
    WsOperation grant=op(WS_GRANT,14,2);CHECK(ws_apply(&s,&r,ctx(101,14,0),grant).status==WS_DENIED);
    CHECK(ws_apply(&s,&r,ctx(101,14,1),grant).status==WS_OK);
    WsOperation decor=op(WS_DECORATE,14,3);CHECK(ws_apply(&s,&r,ctx(102,14,0),decor).status==WS_DENIED);
    CHECK(ws_apply(&s,&r,ctx(101,14,0),decor).status==WS_OK);CHECK(s.entities[14].decor[0]==1&&s.players[0].inventory[0]==1);
    hash=ws_state_hash(&s);CHECK(ws_apply(&s,&r,ctx(101,14,0),decor).status==WS_STALE&&ws_state_hash(&s)==hash);
    WsOperation transfer=op(WS_TRANSFER,16,4);transfer.aux=102;transfer.amount=7;
    CHECK(ws_apply(&s,&r,ctx(101,16,0),transfer).status==WS_OK);CHECK(s.players[1].inventory[1]==107);
    WsContext far=ctx(101,16,0);far.at.pos.x+=100000;transfer=op(WS_EXTRACT,16,5);CHECK(ws_apply(&s,&r,far,transfer).status==WS_DENIED);
    CHECK(ws_apply(&s,&r,ctx(101,16,0),transfer).status==WS_OK);
    size_t n=ws_state_encode(&s,bytes,sizeof(bytes));CHECK(n>0&&ws_state_decode(&loaded,&r,bytes,n)==WS_OK&&ws_state_hash(&s)==ws_state_hash(&loaded));
    ...
    for(int run=0;run<4;run++) {
        setup();int events=histories[run];clock_t start=clock();
        for(int i=0;i<events;i++) {WsOperation o=op((i&1)?WS_REPAIR:WS_DAMAGE,4,(uint32_t)i+1);CHECK(ws_apply(&s,&r,ctx(101,4,1),o).status==WS_OK);}
        uint32_t before=ws_state_hash(&s);s.tail_count=0;n=ws_state_encode(&s,bytes,sizeof(bytes));clock_t restore=clock();CHECK(ws_state_decode(&loaded,&r,bytes,n)==WS_OK);double us=(clock()-restore)*1e6/CLOCKS_PER_SEC;
