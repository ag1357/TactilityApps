/* Instrumented copy of core/render.c for the renderer evaluation.
 * Semantics are unchanged; additions are marked [BENCH]:
 *  - per-stage tick accumulation (transform / clip / setup / raster / clear)
 *  - pixel/depth-traffic counters (bbox tests, covered, depth writes)
 *  - an optional world-space triangle dump (group-tagged) so the Jet
 *    benchmark consumes exactly the same scene geometry
 * The production core/render.c is untouched. */
#include "render.h"
#include "font.inc"
...
/* [BENCH] */
BenchStages bench_st;
int bench_group;
FILE *bench_dump;
static void bench_dump_tri(float ax, float ay, float az, float bx, float by, float bz,
                           float cx, float cy, float cz, unsigned col, float shade) {
    if (!bench_dump) return;
    fprintf(bench_dump, "T %a %a %a %a %a %a %a %a %a %u %a %d\n",
            ax, ay, az, bx, by, bz, cx, cy, cz, col, shade, bench_group);
}
...
static void bench_dump_camera(void) { /* [BENCH] */
    if (!bench_dump) return;
    fprintf(bench_dump, "C %a %a %a %a %a %a\n", rr->camera_x, rr->camera_y, rr->camera_z,
            rr->yaw, rr->pitch, light);
}
...
static void raster(V aa, V bb, V cc, uint32_t col, float shade) {
    uint64_t t0 = bench_now(); /* [BENCH] setup: projection, area, bbox, fog */
    ...
    bench_add(TR_SETUP, bench_now() - t0); /* [BENCH] */
    bench_st.px_tested += (unsigned long long)((maxx - minx + 1) * (maxy - miny + 1)); /* [BENCH] */
    t0 = bench_now(); /* [BENCH] rasterization: per-pixel coverage + depth */
    for (int y = miny; y <= maxy; y++)
        for (int x = minx; x <= maxx; x++) {
            float u = edge(b, c, x + .5f, y + .5f) / area, v = edge(c, a, x + .5f, y + .5f) / area, w = 1 - u - v;
            if (u < 0 || v < 0 || w < 0) continue;
            bench_st.px_covered++; /* [BENCH] */
            ...
            if (depth < rr->depth[pos]) {
                ...
                rr->pixels[pos] = rgb;
                rr->pixels_written++;
                bench_st.depth_writes++; /* [BENCH] */
            }
        }
    bench_add(TR_RASTER, bench_now() - t0); /* [BENCH] */
}
static void tri(V a, V b, V c, uint32_t col, float shade) {
    bench_dump_tri(a.x, a.y, a.z, b.x, b.y, b.z, c.x, c.y, c.z, col, shade); /* [BENCH] */
    uint64_t t0 = bench_now(); /* [BENCH] transform */
    V input[5] = {transform(a), transform(b), transform(c)}, out[5];
    bench_add(TR_TRANSFORM, bench_now() - t0); /* [BENCH] */
    t0 = bench_now(); /* [BENCH] near-plane clip */
    int count = 0;
    for (int i = 0; i < 3; i++) {
    ...
    bench_add(TR_CLIP, bench_now() - t0); /* [BENCH] */
...
void render(Renderer* r, const Game* g) {
    ...
    bench_dump_camera(); /* [BENCH] */
    uint64_t t0 = bench_now(); /* [BENCH] clear */
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++) {
            r->pixels[y * W + x] = color((int)((60 + y * .3f) * light), (int)((88 + y * .3f) * light), (int)((131 + y * .25f) * light));
            r->depth[y * W + x] = 65535;
        }
    bench_add(TR_CLEAR, bench_now() - t0); /* [BENCH] */
    bench_group = 0; /* [BENCH] terrain */
    ...
    bench_group = 3; /* [BENCH] buildings */
    for (int i = 0; i < STRUCT_COUNT; i++) building(g, i);
    bench_group = 4; /* [BENCH] trees */
    for (int i = 0; i < 105; i++) {
    ...
    bench_group = 5; /* [BENCH] evidence and actors */
    for (int i = 0; i < 2; i++)
    ...
    bench_group = 6; /* [BENCH] phos motes */
    for (int i = 0; i < 18; i++) {
...
void render_world(Renderer *r,const WsRecipe *world,WsAddress player,int yaw,WsMetrics *metrics) {
    ...
    bench_add(TR_CLEAR, bench_now() - t0); /* [BENCH] */
    bench_group = 7; /* [BENCH] world modules */
    ...
    if(player.scope==UINT16_MAX) for(uint16_t i=0;i<world->link_count;i++) {
        bench_group = 8; /* [BENCH] trail links */
    ...
    if(player.scope==UINT16_MAX) for(uint16_t f=0;f<world->feature_count;f++) {
        bench_group = 9; /* [BENCH] river strip */
        ...
        uint16_t t0_,t1;
        if(!ws_river_window(world,f,lo,hi,&t0_,&t1))continue;
        WsRiverSample prev;int have=0;
        for(uint32_t t=t0_;t<=t1;t+=2048) {
            WsRiverSample s;
