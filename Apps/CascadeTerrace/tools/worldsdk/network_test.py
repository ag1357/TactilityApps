...
import json, multiprocessing as mp, pathlib, socket, subprocess, sys, tempfile, time
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ct-net-") as temp:
        ...
        try:
            ...
            for _ in range(20):
                check(send(0, {"cmd": "move", "dx": 0, "dz": 100})["ok"])
            for _ in range(10):
                check(send(1, {"cmd": "move", "dx": 100, "dz": 0})["ok"])
            v = snapshot(0)
            ...
            restored = World(ROOT / "build/cascade.cws", save)
            check(restored.snapshot(101)["public_hash"] == expected)
            report = {
                "status": "PASS",
                "checks": checks,
                "clients": 2,
