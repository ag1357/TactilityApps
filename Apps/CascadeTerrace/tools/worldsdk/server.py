...
class Address(C.Structure):
    _fields_ = [("pos", Pos), ("scope", C.c_uint16)]


class Traveler(C.Structure):
    _fields_ = [("at", Address), ("destination", Address), ("remaining", C.c_uint32)]
...
class Player(C.Structure):
    _fields_ = [
        ...
        ("sequence", C.c_uint32),
        ("credits", C.c_uint32),
        ("inventory", C.c_uint16 * 7),
        ("completed", C.c_uint32 * 128),
    ]


class Relationship(C.Structure):
    _fields_ = [
        ("trust", C.c_int16),
        ("reliability", C.c_int16),
        ("cooperation", C.c_int16),
        ("aggression", C.c_int16),
        ("confidence", C.c_uint16),
        ("promise", C.c_uint16),
    ]


class Feed(C.Structure):
    _fields_ = [
        ("revision", C.c_uint32),
        ("actor", C.c_uint32),
        ("epoch", C.c_uint32),
...
class Context(C.Structure):
    _fields_ = [
        ("player", C.c_uint32),
        ("at", Address),
        ("server_authority", C.c_uint8),
    ]


class Disposition(C.Structure):
    _fields_ = [
        ("status", C.c_int),
        ("world_changed", C.c_uint8),
        ("rewarded", C.c_uint8),
        ("historical", C.c_uint8),
        ("reserved", C.c_uint8),
        ("revision", C.c_uint32),
    ]
...
def library():
    ...
    l.ws_load.argtypes = [C.POINTER(Recipe), C.c_void_p, C.c_size_t]
    l.ws_state_init.argtypes = [C.POINTER(State), C.POINTER(Recipe)]
...
class World:
    def __init__(self, product, path):
        ...
        self.lib = library()
        self.recipe = Recipe()
        data = pathlib.Path(product).read_bytes()
        self.product_sha = hashlib.sha256(data).hexdigest()
        if self.lib.ws_load(C.byref(self.recipe), data, len(data)):
            raise ValueError("invalid product")
        self.state = State()
        self.path = str(path).encode()
        self.lib.ws_state_init(C.byref(self.state), C.byref(self.recipe))
        self.lib.ws_restore(C.byref(self.state), C.byref(self.recipe), self.path)
        self.key_path = pathlib.Path(str(path) + ".keys")
        self.keys = (
            json.loads(self.key_path.read_text()) if self.key_path.exists() else {}
        )
        self.positions = {}
        self.connections = set()
        for p in self.state.players[: self.state.player_count]:
            self.positions[p.id] = self.spawn()
    ...
    def checkpoint(self):
        if not self.lib.ws_save(C.byref(self.state), self.path):
    ...
    def hello(self, token):
        if token:
            ...
            if self.lib.ws_join(C.byref(self.state), player):
                raise ValueError("server full")
            self.positions[player] = self.spawn()
            # Server policy assigns the first unowned interior fixture on join.
            for i, m in enumerate(self.recipe.modules[: self.recipe.count]):
                if m.flags & 8 and m.flags & 64 and not self.state.entities[i].owner:
                    e = self.state.entities[i]
                    result = self.lib.ws_apply(
                        C.byref(self.state),
                        C.byref(self.recipe),
                        Context(player, self.positions[player].at, 1),
                        Operation(1, e.epoch, e.revision, 5, i, 1, 0),
                    )
                    if result.status:
                        raise ValueError("property provisioning failed")
                    break
            token = secrets.token_hex(24)
            self.keys[token] = player
    ...
    def snapshot(self, player):
        ...
        public = {
            "ancestry": list(self.state.ancestry.word),
            "recipe_sha256": self.product_sha,
            ...
            "entities": [
                ...
                for e in self.state.entities[: self.state.count]
            ],
            "feed": [
                {
                    "revision": f.revision,
        ...
        p = next(
            p for p in self.state.players[: self.state.player_count] if p.id == player
        )
        return {
            "world": public,
            "public_hash": digest,
    ...
    def dispatch(self, player, msg):
        ...
        if cmd == "move":
            dx, dz = msg["dx"], msg["dz"]
            if type(dx) != int or type(dz) != int or abs(dx) > 1000 or abs(dz) > 1000:
                raise ValueError("movement bounds")
            traveler = self.positions[player]
            if traveler.remaining:
                self.lib.ws_travel_tick(C.byref(traveler), 16)
            else:
                status = (
                    0
                    if self.lib.ws_move(
                        C.byref(self.recipe), C.byref(traveler.at), dx, dz
                    )
                    else 7
                )
        elif cmd == "use":
            ...
            if not self.lib.ws_use_link(C.byref(self.recipe), C.byref(candidate)):
                ...
                if (
                    scope != 65535
                    and self.state.entities[scope].owner not in (0, player)
                    and not self.state.entities[scope].public_access
                ):
                    status = 7
                else:
                    self.positions[player] = candidate
        elif cmd == "act":
            if msg.get("recipe_sha256") != self.product_sha:
            ...
            if any(
                type(v) != int or v < 0 or v > (0xFFFFFFFF if i < 3 else 65535)
                for i, v in enumerate(values)
            ):
                raise ValueError("operation bounds")
            ...
            d = self.lib.ws_apply(
                C.byref(self.state),
                C.byref(self.recipe),
                Context(player, self.positions[player].at, 0),
                op,
            )
            status = d.status
            if not status:
                try:
                    ...
                    C.memmove(C.byref(self.state), previous, len(previous))
                    raise
        elif cmd != "snapshot":
        ...
        try:
            while True:
                ...
                with self.server.world.lock:
                    if player is None:
                        if msg.get("cmd") != "hello" or msg.get("schema") != 1:
                            raise ValueError("handshake required")
                        player, token = self.server.world.hello(msg.get("token"))
                        reply = {
                            "ok": True,
                            "token": token,
                            **self.server.world.snapshot(player),
                        }
                    else:
                        reply = self.server.world.dispatch(player, msg)
                revision = reply["world"]["revision"]
                reply["world_revision"] = revision
                if msg.get("cmd") == "move" and revision == last_revision:
                    reply.pop("world")
            ...
            try:
                self.wfile.write(
                    b'{"ok":false,"error":"protocol or checkpoint failure"}\n'
                )
            except OSError:
                pass
        finally:
            if player:
                with self.server.world.lock:
                    self.server.world.connections.discard(player)
...
if __name__ == "__main__":
    ...
    a.add_argument("--save", default="build/world-server")
