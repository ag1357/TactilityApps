...
static void part(WsBoxFn fn,void *ctx,WsPos p,WsPos s,uint32_t c) { if(fn) fn(ctx,p,s,c); }
void ws_boxes(const WsModule *m,WsBoxFn fn,void *ctx) {
    WsPos p=m->pos,s=m->size;
    if((m->kind&255)==WS_ROOM_MODULE) {
        WsPos a=s; a.y=300; part(fn,ctx,p,a,m->color);
        a=(WsPos){300,s.y,s.z}; p.x=m->pos.x-s.x/2+150; part(fn,ctx,p,a,m->color);
        p.x=m->pos.x+s.x/2-150; part(fn,ctx,p,a,m->color);
        p=m->pos; p.z-=s.z/2-150; a=(WsPos){s.x,s.y,300}; part(fn,ctx,p,a,m->color);
        a.x=(s.x-4000)/2;
        if(a.x>0) { p.z=m->pos.z+s.z/2-150; p.x=m->pos.x-(s.x+4000)/4; part(fn,ctx,p,a,m->color); p.x=m->pos.x+(s.x+4000)/4; part(fn,ctx,p,a,m->color); }
    } else if((m->kind&255)==WS_RAMP) {
        /* Eight explicit traversable steps; bounded primitive vocabulary. */
        s.z=m->size.z/8; if(s.z<1) s.z=1;
        for(int i=0;i<8;i++) { p.z=m->pos.z-m->size.z/2+s.z/2+i*s.z; s.y=m->size.y*(i+1)/8; if(!s.y)s.y=1; part(fn,ctx,p,s,m->color); }
    } else part(fn,ctx,p,s,m->color);
}
typedef struct { WsPos p; int32_t radius; int hit; } Collision;
static void collide(void *ctx,WsPos p,WsPos s,uint32_t color) {
    (void)color; Collision *c=ctx;
...
int ws_collision(const WsRecipe *r,WsAddress at,int32_t radius) {
    ...
    for(uint16_t i=0;i<r->count;i++) {
        ...
        if(src->flags&WS_SOLID) { WsModule m; ws_materialize(r,i,&m); ws_boxes(&m,collide,&c); }
    }
    return c.hit;
}
int ws_surface(const WsRecipe *r,WsAddress at,int32_t ceiling,int32_t *height) {
    int found=0; int32_t best=INT32_MIN;
    for(uint16_t i=0;i<r->count;i++) {
        const WsModule *src=&r->modules[i];
        ...
        WsModule m; ws_materialize(r,i,&m);
        if(llabs((int64_t)at.pos.x-m.pos.x)>m.size.x/2||llabs((int64_t)at.pos.z-m.pos.z)>m.size.z/2) continue;
        int32_t y=m.pos.y;
        if((m.kind&255)==WS_RAMP) { int64_t t=(int64_t)at.pos.z-m.pos.z+m.size.z/2; int k=(int)(t*8/m.size.z); if(k>7)k=7; y+=m.size.y*(k+1)/8; }
        else y+=((m.kind&255)==WS_ROOM_MODULE?300:m.size.y);
        if(y<=ceiling&&y>best) { best=y; found=1; }
    }
...
uint16_t ws_witness(const WsRecipe *r,WsAddress observer,WsAddress event,uint16_t attention,uint16_t conspicuous) {
    ...
    int64_t d=llabs(dx)+llabs(dy)+llabs(dz); if(d>=30000)return 0;
    for(int i=1;i<128;i++) { WsAddress p=observer; p.pos=(WsPos){observer.pos.x+(int32_t)(dx*i/128),observer.pos.y+(int32_t)(dy*i/128),observer.pos.z+(int32_t)(dz*i/128)}; if(ws_collision(r,p,0))return 0; }
