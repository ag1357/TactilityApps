...
int ws_route(const WsRecipe *r,uint16_t a,uint16_t b,const uint8_t *blocked,uint16_t *path,size_t cap) {
    ...
    queue[tail++]=a; prev[a]=a;
    while(head<tail) {
        uint16_t at=queue[head++]; if(at==b) break;
        for(int i=0;i<r->link_count;i++) {
            const WsLink *l=&r->links[i]; uint16_t next=l->a==at?l->b:l->b==at?l->a:UINT16_MAX;
            if(next>=r->count||prev[next]!=UINT16_MAX||(blocked&&blocked[next])) continue;
            prev[next]=at; queue[tail++]=next;
        }
    }
    if(prev[b]==UINT16_MAX) return 0;
    for(uint16_t at=b;;at=prev[at]) { queue[n++]=at; if(at==a) break; }
...
WsError ws_validate(const WsRecipe *r) {
    ...
    for(int i=0;i<r->count;i++) {
        ...
        for(int j=0;j<i;j++) if(ws_id_equal(m->id,r->modules[j].id)) return WS_DUPLICATE;
    }
    for(int i=0;i<r->link_count;i++) {
    ...
    int first=-1; uint16_t path[WS_CAP];
    for(int i=0;i<r->count;i++) if(r->modules[i].flags&WS_WALK) {
        if(first<0) first=i;
        else if(ws_route(r,(uint16_t)first,(uint16_t)i,0,path,WS_CAP)<=0) return WS_DISCONNECTED;
    }
    return WS_OK;
}
WsError ws_load(WsRecipe *r,const uint8_t *p,size_t n) {
    ...
    r->seed=u32(p+32); r->epoch=u32(p+36); r->revision=u32(p+40); r->recipe_crc=u32(p+12);
    r->count=(uint16_t)count; r->link_count=(uint16_t)links; p+=48;
    for(unsigned i=0;i<count;i++,p+=64) {
        WsModule *m=&r->modules[i]; for(int j=0;j<4;j++) m->id.word[j]=u32(p+j*4);
        m->parent=u16(p+16); m->kind=u16(p+18); m->flags=u16(p+20); m->phos=u16(p+22);
        m->pos=(WsPos){i32(p+24),i32(p+28),i32(p+32)}; m->size=(WsPos){i32(p+36),i32(p+40),i32(p+44)};
        m->color=u32(p+48); m->seed=u32(p+52); m->jitter_x=(int16_t)u16(p+56); m->jitter_z=(int16_t)u16(p+58); m->affinity=u16(p+60); m->quantity=u16(p+62);
    }
    for(unsigned i=0;i<links;i++,p+=8) r->links[i]=(WsLink){u16(p),u16(p+2),u16(p+4),u16(p+6)};
    WsError e=ws_validate(r); if(e!=WS_OK) memset(r,0,sizeof(*r)); return e;
}
