#include "sdk.h"
uint32_t ws_hash(uint32_t x) { x^=x>>16; x*=0x7feb352dU; x^=x>>15; x*=0x846ca68bU; return x^(x>>16); }
...
WsId ws_child_id(WsId p,uint32_t key,uint32_t version) {
    WsId r; for(int i=0;i<4;i++) r.word[i]=ws_hash(p.word[i]^ws_hash(key+0x9e3779b9U*(uint32_t)(i+1))^version); return r;
}
