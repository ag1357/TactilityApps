...
/* Regional reservoirs (schema 3): bounded regional stocks of terrain
   material, water, biomass and Phos. A reservoir is a bookkeeping record,
   not a fluid simulation: drawdown changes what river samples report inside
   the region (ws_river_stage), never geometry outside it. `rate` is the
   base recovery per in-game day (86400 s of state clock), scaled by the
   geology/biome class, the deterministic regional weather and, for biomass,
   the paired Phos stock. */
#define WS_RESERVOIR_CAP 8
enum { WS_RES_TERRAIN = 1, WS_RES_WATER = 2, WS_RES_BIOMASS = 3, WS_RES_PHOS = 4 };
typedef struct {
    WsId id;
    uint16_t kind, zone; /* kind: WS_RES_*; zone: recovery class 0..3 */
