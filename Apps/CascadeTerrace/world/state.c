...
#include <stdlib.h>
#include <string.h>
static int player(const WsState *s,uint32_t id) { for(int i=0;i<s->player_count;i++)if(s->players[i].id==id)return i;return -1; }
void ws_state_init(WsState *s,const WsRecipe *r) {
    memset(s,0,sizeof(*s));s->ancestry=r->ancestry;s->recipe_crc=r->recipe_crc;s->revision=r->revision;s->count=r->count;
    for(int i=0;i<r->count;i++){s->entities[i].epoch=1;s->entities[i].health=(r->modules[i].flags&WS_REPAIRABLE)?0:100;s->entities[i].alive=1;s->entities[i].public_access=1;s->entities[i].quantity=r->modules[i].quantity;}
}
WsError ws_join(WsState *s,uint32_t id) {
    if(!id)return WS_DENIED;if(player(s,id)>=0)return WS_OK;
    if(s->player_count>=WS_PLAYER_CAP||s->revision==UINT32_MAX)return WS_FULL;
    s->players[s->player_count++].id=id;s->revision++;return WS_OK;
}
...
static void record(WsState *s,WsContext c,WsOperation op,uint16_t kind) {
    if(s->tail_count==WS_TAIL_CAP){memmove(s->tail,s->tail+1,sizeof(*s->tail)*(WS_TAIL_CAP-1));s->tail_count--;}
    s->tail[s->tail_count++]=op;
    if(s->feed_count==WS_FEED_CAP){memmove(s->feed,s->feed+1,sizeof(*s->feed)*(WS_FEED_CAP-1));s->feed_count--;}
...
static int near(const WsRecipe *r,WsContext c,uint16_t target) {
    WsModule m;ws_materialize(r,target,&m);
...
WsDisposition ws_apply(WsState *s,const WsRecipe *r,WsContext c,WsOperation op) {
    ...
    if(ws_state_validate(s,r)!=WS_OK){d.status=WS_VERSION;return d;}
    int pi=player(s,c.player);if(pi<0||op.target>=s->count)return d;
    WsPlayer *p=&s->players[pi];WsEntity *e=&s->entities[op.target];const WsModule *m=&r->modules[op.target];
    if(op.sequence<=p->sequence||op.epoch!=e->epoch||op.base_revision!=e->revision){d.status=WS_STALE;return d;}
    if(!op.sequence||s->revision==UINT32_MAX){d.status=WS_FULL;return d;}
    ...
    case WS_EXTRACT:
        ...
        e->quantity-=op.amount;p->inventory[m->phos]+=op.amount;break;
    case WS_REPAIR:
        if(!(m->flags&WS_REPAIRABLE)||e->health==100||!p->inventory[m->phos]||p->completed[op.target]>=e->epoch||p->credits>UINT32_MAX-10)return d;
        p->inventory[m->phos]--;e->health=100;p->completed[op.target]=e->epoch;p->credits+=10;d.rewarded=1;break;
    case WS_DAMAGE:
        if(!c.server_authority||!(m->flags&WS_REPAIRABLE)||e->epoch==UINT32_MAX)return d;
        e->health=0;e->epoch++;break;
    case WS_TRANSFER: {
        int to=player(s,op.aux);if(to<0||to==pi||m->phos>=WS_INVENTORY||!op.amount||p->inventory[m->phos]<op.amount||s->players[to].inventory[m->phos]>UINT16_MAX-op.amount)return d;
        p->inventory[m->phos]-=op.amount;s->players[to].inventory[m->phos]+=op.amount;break;
    }
    case WS_GRANT:
        if(!c.server_authority||!(m->flags&WS_PROPERTY)||e->owner)return d;e->owner=c.player;e->public_access=0;break;
    case WS_DECORATE:
        ...
        if(op.amount)p->inventory[op.amount-1]--;if(e->decor[op.aux])p->inventory[e->decor[op.aux]-1]++;e->decor[op.aux]=op.amount;break;
    case WS_AID:
        if(!(m->flags&WS_NPC)||!e->alive||p->completed[op.target]>=e->epoch||!p->inventory[m->phos]||p->credits>UINT32_MAX-10)return d;
        p->inventory[m->phos]--;p->completed[op.target]=e->epoch;p->credits+=10;d.rewarded=1;break;
    case WS_KILL:
        if(!c.server_authority||!(m->flags&WS_NPC)||!e->alive||e->epoch==UINT32_MAX)return d;e->alive=0;e->epoch++;break;
    case WS_PROMISE: case WS_KEEP_PROMISE: {
        int to=player(s,op.aux);if(to<0||to==pi)return d;WsRelationship *rel=&s->relations[to][pi];
        if(op.action==WS_PROMISE){if(rel->promise)return d;rel->promise=1;}
        else {if(!rel->promise)return d;rel->promise=0;if(rel->reliability<990)rel->reliability+=10;if(rel->confidence<990)rel->confidence+=10;}break;
    }
    default:return d;
    }
    s->revision++;e->revision=s->revision;p->sequence=op.sequence;d.status=WS_OK;d.world_changed=1;d.revision=s->revision;
    record(s,c,op,op.action==WS_TRANSFER?WS_TRADE:WS_WORLD_EVENT);return d;
}
WsDisposition ws_merge(WsState *s,const WsState *base,const WsRecipe *r,WsContext c,WsOperation op) {
    ...
    int pi=player(s,c.player),bp=player(base,c.player);if(pi<0||bp<0||op.target>=s->count)return d;
    const WsEntity *b=&base->entities[op.target];WsEntity *e=&s->entities[op.target];WsPlayer *p=&s->players[pi];uint16_t phos=r->modules[op.target].phos;
    ...
    if(op.action==WS_REPAIR&&e->epoch!=b->epoch){d.status=WS_STALE;return d;}
    if(s->revision==UINT32_MAX||p->credits>UINT32_MAX-10){d.status=WS_FULL;return d;}
    d.historical=(uint8_t)(e->epoch!=b->epoch);
    if(op.action==WS_REPAIR&&e->health!=100){e->health=100;d.world_changed=1;}
    /* Server death is never reversed. Historical aid only earns personal credit. */
    p->inventory[phos]--;p->completed[op.target]=op.epoch;p->credits+=10;d.rewarded=1;
    if(op.sequence>p->sequence)p->sequence=op.sequence;
    s->revision++;if(d.world_changed)e->revision=s->revision;
    d.status=WS_OK;d.revision=s->revision;record(s,c,op,WS_DISCOVERY);return d;
}
