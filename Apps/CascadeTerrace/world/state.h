...
typedef enum { WS_EXTRACT=1,WS_REPAIR,WS_DAMAGE,WS_TRANSFER,WS_GRANT,WS_DECORATE,WS_AID,WS_KILL,WS_PROMISE,WS_KEEP_PROMISE } WsAction;
typedef enum { WS_BOUNTY,WS_TRADE,WS_PLAYER_NOTE,WS_WORLD_EVENT,WS_RUMOR,WS_FACTION_NOTICE,WS_DISCOVERY,WS_NEWS_RESERVED } WsFeedKind;
typedef struct {
...
typedef struct { int16_t trust,reliability,cooperation,aggression; uint16_t confidence,promise; } WsRelationship;
typedef struct { uint32_t revision,actor,epoch; uint16_t target,kind; } WsFeed;
typedef struct { uint32_t sequence,epoch,base_revision; uint16_t action,target,amount,aux; } WsOperation;
typedef struct {
    WsId ancestry;
...
typedef struct { uint32_t player; WsAddress at; uint8_t server_authority; } WsContext;
typedef struct { WsError status; uint8_t world_changed,rewarded,historical,reserved; uint32_t revision; } WsDisposition;
